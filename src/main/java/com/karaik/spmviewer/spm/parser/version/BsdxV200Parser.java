...
@Slf4j
public class BsdxV200Parser implements SpmDialectParser {
    ...
    protected Spm.SPMHitArea parseHit(BinaryReader reader) {
        try {
            var hit = new LegacyRectHitArea();
            hit.setId((short) -1);
            hit.setShapeType((short) -1);
            hit.setUnk0(reader.readInt());
            hit.setHitRect(parseRect(reader));
    ...
    protected Spm.SPMAnimData parseAnim(BinaryReader reader, int patPageNum) {
        ...
        int numPat = anim.getNumPat() == null ? 0 : (anim.getNumPat() & 0xFFFF);
